...
final videotoken=StateProvider((ref) => "");
...
class SheduleCallProvider extends ChangeNotifier {
  ...
  getCall(context, {required WidgetRef ref}) async {
    ...
    try {
      final response = await dio.post(
        "${AppConstants.BASE_URL_NEW}get_call/${userID}",
      );
      if (response.statusCode == 200) {
        ...
        if (message['message'] == "Call joined successfully") {
          isApiLoading = false;
       notifyListeners();
       ref.read(videotoken.notifier).update((state) => message["token"]);
          // Navigator.push(
          //     context,
          //     MaterialPageRoute(
 ...
 Get.snackbar("Scheduled Succesfully","Your slot have successfully Scheduled", backgroundColor: Colors.green, colorText: Colors.white);
      } else {
        load = false;
        print("error");
      }
