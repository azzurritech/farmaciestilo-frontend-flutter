...
import 'package:farmacie_stilo/data/model/response/distance_model.dart';
import 'package:farmacie_stilo/data/model/response/order_cancellation_body.dart';
...
import 'package:farmacie_stilo/data/model/response/refund_model.dart';
...
import 'package:farmacie_stilo/data/model/response/timeslote_model.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> placePrescriptionOrder(
     String itemid,
      ...
      String latitude,
      String note,
      List<XFile> orderAttachment, Function(bool isSuccess, String? message, String orderID, int? zoneID,
    ...
    Response response = await orderRepo.placePrescriptionOrder(
        storeId, distance, itemid,address, longitude, latitude, note,orderAmount.toString() ,multiParts);
    _isLoading = false;
    if (response.statusCode == 200) {
      String? message = response.body['message'];
      String orderID = response.body['order_id'].toString();
      ...
      callback(
          false, 'store_is_closed'.tr, '-1', zoneID, orderAmount, maxCodAmount);
    }
    update();
  }
  ...
  Future<double?> getDistanceInKM(LatLng originLatLng, LatLng destinationLatLng,
      {bool isDuration = false, bool isRiding = false}) async {
    _distance = -1;
