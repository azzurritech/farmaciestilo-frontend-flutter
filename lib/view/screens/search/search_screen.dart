...
import 'package:farmacie_stilo/view/screens/search/widget/search_result_widget.dart';
import 'package:flutter/material.dart' hide SearchController;
import 'package:get/get.dart';
import 'package:farmacie_stilo/view/screens/store/widget/bottom_cart_widget.dart';
...
class SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if (_isLoggedIn) {
      Get.find<SearchControllers>().getSuggestedItems();
    }
    Get.find<SearchControllers>().getHistoryList();
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (Get.find<SearchControllers>().isSearchMode) {
          return true;
        } else {
          Get.find<SearchControllers>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
          ...
          body: SafeArea(
            ...
            child: GetBuilder<SearchControllers>(builder: (searchController) {
  ...
  void _actionSearch(bool isSubmit, String? queryText, bool fromHome) {
    if (Get.find<SearchControllers>().isSearchMode || isSubmit) {
      if (queryText!.isNotEmpty) {
        Get.find<SearchControllers>().searchData(queryText, fromHome);
      } else {
      ...
      if (!Get.find<SearchControllers>().isStore) {
        for (var product in Get.find<SearchControllers>().allItemList!) {
      ...
      Get.dialog(FilterWidget(
          maxValue: maxValue, isStore: Get.find<SearchControllers>().isStore));
    }
  }
}
